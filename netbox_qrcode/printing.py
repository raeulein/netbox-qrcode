...
def _prepare_image(raw_png: BytesIO, label_code: str) -> BytesIO:
    """Skaliere PNG exakt auf das vom Labelcode erwartete Raster (keine Rotation)."""
...
def print_png(image: BytesIO, label_size: str | None = None) -> None:
    """PNG → Brother-Raster → Drucker senden (Rotation = auto)."""
    ...
    instr    = convert(raster, [img_prepared], label=label, rotate="auto")

    be_info       = backend_factory(p_cfg["BACKEND"])          # z. B. "network"
    backend_obj   = be_info["backend_class"](p_cfg["ADDRESS"]) # tcp://…
